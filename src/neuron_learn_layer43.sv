...
module neuron_learn_layer43 #(
    ...
    output unit_t out [43-1:0],

    output frac_t weights [43-1:0][N-1:0],
    output frac_t activation_max [43-1:0],
    output frac_t activation_min [43-1:0],

    input unit_t expected_out [43-1:0],
    output unit_t expected_in [N-1:0]
);

    unit_t unavg_expected_in [43-1:0][N-1:0];
    unit_arr_average #(.N(43), .LEN(N)) avg (.in(unavg_expected_in), .out(expected_in));
