...
module neuron_learn_layer26 #(
    ...
    output unit_t out [26-1:0],

    output unit_signed_t weights [26-1:0][N-1:0],
    output unit_signed_t activation_upper_bound [26-1:0],
    output unit_signed_t activation_lower_bound [26-1:0],

    input unit_t expected_out [26-1:0],
    output unit_t expected_in [N-1:0]
);

    unit_t unavg_expected_in [26-1:0][N-1:0];
    unit_arr_average #(.N(26), .LEN(N)) avg (.in(unavg_expected_in), .out(expected_in));
