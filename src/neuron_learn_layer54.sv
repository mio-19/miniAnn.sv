...
module neuron_learn_layer54 #(
    ...
    output unit_t out [54-1:0],

    output frac_t weights [54-1:0][N-1:0],
    output frac_t activation_max [54-1:0],
    output frac_t activation_min [54-1:0],

    input unit_t expected_out [54-1:0],
    output unit_t expected_in [N-1:0]
);

    unit_t unavg_expected_in [54-1:0][N-1:0];
    unit_arr_average #(.N(54), .LEN(N)) avg (.in(unavg_expected_in), .out(expected_in));
