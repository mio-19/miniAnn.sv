...
module neuron_learn_layer23 #(
    ...
    output unit_t out [23-1:0],

    output unit_signed_t weights [23-1:0][N-1:0],
    output unit_signed_t activation_upper_bound [23-1:0],
    output unit_signed_t activation_lower_bound [23-1:0],

    input unit_t expected_out [23-1:0],
    output unit_t expected_in [N-1:0]
);

    unit_t unavg_expected_in [23-1:0][N-1:0];
    unit_arr_average #(.N(23), .LEN(N)) avg (.in(unavg_expected_in), .out(expected_in));
