...
module neuron_learn_layer13 #(
    ...
    output unit_t out [13-1:0],

    output unit_signed_t weights [13-1:0][N-1:0],
    output unit_signed_t activation_upper_bound [13-1:0],
    output unit_signed_t activation_lower_bound [13-1:0],

    input unit_t expected_out [13-1:0],
    output unit_t expected_in [N-1:0]
);

    unit_t unavg_expected_in [13-1:0][N-1:0];
    unit_arr_average #(.N(13), .LEN(N)) avg (.in(unavg_expected_in), .out(expected_in));
