...
module neuron_learn_layer37 #(
    ...
    output unit_t out [37-1:0],

    output unit_signed_t weights [37-1:0][N-1:0],
    output unit_signed_t activation_upper_bound [37-1:0],
    output unit_signed_t activation_lower_bound [37-1:0],

    input unit_t expected_out [37-1:0],
    output unit_t expected_in [N-1:0]
);

    unit_t unavg_expected_in [37-1:0][N-1:0];
    unit_arr_average #(.N(37), .LEN(N)) avg (.in(unavg_expected_in), .out(expected_in));
