...
module neuron_learn_layer47 #(
    ...
    output unit_t out [47-1:0],

    output frac_t weights [47-1:0][N-1:0],
    output frac_t activation_max [47-1:0],
    output frac_t activation_min [47-1:0],

    input unit_t expected_out [47-1:0],
    output unit_t expected_in [N-1:0]
);

    unit_t unavg_expected_in [47-1:0][N-1:0];
    unit_arr_average #(.N(47), .LEN(N)) avg (.in(unavg_expected_in), .out(expected_in));
