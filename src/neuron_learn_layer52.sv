...
module neuron_learn_layer52 #(
    ...
    output unit_t out [52-1:0],

    output frac_t weights [52-1:0][N-1:0],
    output frac_t activation_max [52-1:0],
    output frac_t activation_min [52-1:0],

    input unit_t expected_out [52-1:0],
    output unit_t expected_in [N-1:0]
);

    unit_t unavg_expected_in [52-1:0][N-1:0];
    unit_arr_average #(.N(52), .LEN(N)) avg (.in(unavg_expected_in), .out(expected_in));
